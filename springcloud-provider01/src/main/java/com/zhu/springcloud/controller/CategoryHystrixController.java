...
@RequestMapping("hystrix/category")
public class CategoryHystrixController {


    @Autowired
    ICategoryInfoService categoryInfoServiceImpl;

    public List <CategoryInfo> test(){
        System.out.println("------调用熔断调用等待方法");

        return   categoryInfoServiceImpl.list(null);

    }

    @HystrixCommand(fallbackMethod = "test") //fallbackMethod 调用的其他方法名 5秒 20 次试
    @GetMapping("getList")
